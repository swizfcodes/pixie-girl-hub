...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("RetailPartners");
  return item;
}
...
async function create({ brand, user, request_id, input }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "retail_partners.create",
      target_type: "retail_partners",
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("RetailPartners");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "retail_partners.update",
      target_type: "retail_partners",
      target_id: id,
      before,
      after: updated,
...
async function archive({ brand, user, request_id, id }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "retail_partners.archive",
      target_type: "retail_partners",
      target_id: id,
      before,
      request_id,
