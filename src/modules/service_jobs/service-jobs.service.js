...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("ServiceJobs");
  return item;
}
...
async function create({ brand, user, request_id, input }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "service_jobs.create",
      target_type: "service_jobs",
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("ServiceJobs");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "service_jobs.update",
      target_type: "service_jobs",
      target_id: id,
      before,
      after: updated,
...
async function archive({ brand, user, request_id, id }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "service_jobs.archive",
      target_type: "service_jobs",
      target_id: id,
      before,
      request_id,
