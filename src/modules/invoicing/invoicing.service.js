...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("Invoicing");
  return item;
}
...
async function create({ brand, user, request_id, input }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "invoicing.create",
      target_type: "invoicing",
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("Invoicing");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "invoicing.update",
      target_type: "invoicing",
      target_id: id,
      before,
      after: updated,
