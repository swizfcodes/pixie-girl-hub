...
function tableFor(brand) {
  ...
  return `${brand}.storefront_themes`;
}
...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE storefront_themes_id = $1 LIMIT 1`;
  // NOTE: replace `storefront_themes_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
