...
function tableFor(brand) {
  ...
  return `${brand}.staff_clock_events`;
}
...
async function findById({ client, brand, id }) {
  const sql = `SELECT * FROM ${tableFor(brand)} WHERE staff_clock_events_id = $1 LIMIT 1`;
  // NOTE: replace `staff_clock_events_id` with the actual PK name for this table
  const exec = client ? client.query.bind(client) : query;
