...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("Attendance");
  return item;
}
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("Attendance");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "attendance.update",
      target_type: "attendance",
      target_id: id,
      before,
      after: updated,
