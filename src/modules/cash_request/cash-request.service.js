...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("CashRequest");
  return item;
}
...
async function create({ brand, user, request_id, input }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "cash_request.create",
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("CashRequest");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "cash_request.update",
      target_type: "cash_request",
      target_id: id,
      before,
      after: updated,
...
async function archive({ brand, user, request_id, id }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "cash_request.archive",
      target_type: "cash_request",
      target_id: id,
      before,
      request_id,
