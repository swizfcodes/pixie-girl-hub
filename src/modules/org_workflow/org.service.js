...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("OrgWorkflow");
  return item;
}
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("OrgWorkflow");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "org_workflow.update",
      target_type: "org_workflow",
      target_id: id,
      before,
      after: updated,
...
async function archive({ brand, user, request_id, id }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "org_workflow.archive",
      target_type: "org_workflow",
      target_id: id,
      before,
      request_id,
