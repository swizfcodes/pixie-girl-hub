...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("AiInsights");
  return item;
}
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("AiInsights");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "ai_insights.update",
      target_type: "ai_insights",
      target_id: id,
      before,
      after: updated,
...
async function archive({ brand, user, request_id, id }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "ai_insights.archive",
      target_type: "ai_insights",
      target_id: id,
      before,
      request_id,
