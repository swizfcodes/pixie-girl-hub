...
async function getById({ brand, user, scope, id }) {
  ...
  if (!item) throw new NotFoundError("HrPayroll");
  return item;
}
...
async function create({ brand, user, request_id, input }) {
  return transaction(async (client) => {
    ...
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "hr_payroll.create",
      target_type: "hr_payroll",
...
async function update({ brand, user, request_id, id, patch }) {
  return transaction(async (client) => {
    ...
    if (!before) throw new NotFoundError("HrPayroll");
    const updated = await repo.update({ client, brand, id, patch });
    await audit({
      business: brand,
      user_id: user.user_id,
      action_key: "hr_payroll.update",
      target_type: "hr_payroll",
      target_id: id,
      before,
      after: updated,
